...
## Known limitation — single-account coverage
All sample HTML comes from one student's account. The portal may render
different table columns, extra sections, or different menu items depending
on year/branch/backlog status/hostel status etc. CSS targets structure
(classes/tags), not data, so most content differences (subject names,
attendance numbers) won't matter — but *structural* differences (extra
columns, conditionally-rendered sections) could cause visual bugs for other
students.

This is a known, accepted v1 limitation, not an oversight: ship against the
one account we have, treat "looks broken for a friend in a different
year/branch" as a bug report to fix incrementally, not a blocker to launch.
When that happens, grab a saved-HTML sample from that specific case and
diff it against what we already have to find the new selector(s) needed.
