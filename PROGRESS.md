...
- Fixed `chrome.storage.sync` crash for new users who lacked the `reecapLoginRole` variable configured in their environment by ensuring default properties object is initialized properly inside `chrome.storage.sync.get()`, and added fallback regex ID checking in case of storage unreliability.
